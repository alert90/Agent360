...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const {
      ...
      code,
      description,
      type = 'percentage', // Default to percentage only
      value,
      agentType = 'all',
      status = 'active',
      ...
      assignedUsers = [] // New field for user-specific assignments
    } = req.body
    ...
    if (configId) {
      // Get the complete configuration with assignments
      const newConfig = db
        .prepare(
          `
        SELECT
      ...
      const configWithUsers = {
        ...(newConfig as any),
        kpi_weights: (newConfig as any).kpi_weights ? JSON.parse((newConfig as any).kpi_weights) : {},
        assigned_user_ids: (newConfig as any).assigned_user_ids
          ? (newConfig as any).assigned_user_ids.split(',').map(Number)
          : []
      }

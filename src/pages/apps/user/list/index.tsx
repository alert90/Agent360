...
const RowOptions = ({ id, onDeleteUser }: { id: number | string; onDeleteUser: (id: number | string) => void }) => {
  // ** State
...
const UserList = ({ apiData }: InferGetStaticPropsType<typeof getStaticProps>) => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        const response = await axios.get('/api/apps/users/list')
  ...
  const handleDeleteUser = useCallback(async (id: number | string) => {
    try {
      const response = await fetch('/api/apps/users/delete', {
        method: 'DELETE',
        headers: {
  ...
  const handleAddUser = useCallback((newUser: UsersType) => {
  ...
  const columns: GridColDef[] = [
      ...
      renderCell: ({ row }: CellType) => {
        return (
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            ...
            >
              <Icon icon={userRoleObj[row.role]?.icon || 'tabler:user'} />
            </CustomAvatar>
    ...
    },
    {
      ...
      field: 'actions',
      headerName: 'Actions',
      renderCell: ({ row }: CellType) => <RowOptions id={row.id} onDeleteUser={handleDeleteUser} />
    }
  ]
  ...
  return (
    <Grid container spacing={6.5}>
      ...
      <AddUserDrawer open={addUserOpen} toggle={toggleAddUserDrawer} onUserAdded={handleAddUser} />
    </Grid>
  )
}
...
export const getStaticProps: GetStaticProps = async () => {
  try {
    const res = await axios.get('http://localhost:3000/api/cards/statistics')

...
// ** Complete mock data matching InvoiceType
const defaultInvoices: InvoiceType[] = [
  {
    id: 1,
    name: 'John Doe',
    avatar: '',
  ...
  },
  {
    ...
    avatar: '',
    avatarColor: 'error',
    companyEmail: 'services@global.com',
    total: 4200,
    ...
    address: '654 International Dr, Miami, FL 33101',
...
const InvoiceList = () => {
  ...
  const [data, setData] = useState<InvoiceType[]>(defaultInvoices)

  // ** Filter data based on search and filters
  useEffect(() => {
    let filteredData = defaultInvoices
    ...
    if (value) {
      filteredData = filteredData.filter(
        invoice =>
          ...
          invoice.companyEmail.toLowerCase().includes(value.toLowerCase()) ||
          invoice.company.toLowerCase().includes(value.toLowerCase())
      )
    }
    ...
    if (statusValue) {
      filteredData = filteredData.filter(invoice =>
        invoice.invoiceStatus.toLowerCase().includes(statusValue.toLowerCase())
      )
    }
      ...
      renderCell: ({ row }: CellType) => (
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          <Tooltip title='Delete Invoice'>
            <IconButton size='small' sx={{ color: 'text.secondary' }} onClick={() => handleDeleteInvoice(row.id)}>
